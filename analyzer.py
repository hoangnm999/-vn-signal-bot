...
# ══════════════════════════════════════════════════════════════════════════════
# ALIASES — export các hàm internal để các module khác import trực tiếp
# ══════════════════════════════════════════════════════════════════════════════

def get_indicators(symbol: str, df=None) -> dict:
    """
    Alias công khai của compute_indicators.
    Nếu df=None sẽ tự lấy price data trước.
    Dùng bởi local_swarm_cmd, backtest_rule_cmd, bot.py.
    """
    import pandas as pd
    if df is None:
        price = get_price_data(symbol, days=200)
        if not price.get("success"):
            return {}
        df = price["df"]
    # compute_indicators nhận DataFrame, không nhận symbol
    if isinstance(df, dict):
        # nếu nhận nhầm dict thay vì df
        df = df.get("df", None)
        if df is None:
            return {}
    try:
        ...
        _logger.warning("get_indicators error %s: %s", symbol, e)
        return {}


def run_vibe_agents(symbol: str, df=None) -> dict:
    """
    Alias công khai để chạy các vibe_skills agents từ bên ngoài.
    Trả về dict với keys: signals, bull, bear, details, v.v.
    Dùng bởi bot.py khi inject context vào Vibe-Trading swarm.
    """
    try:
        from vibe_skills import run_vibe_agents as _rva
        if df is None:
            price = get_price_data(symbol, days=300)
            if not price.get("success"):
                return {"bull": 0, "bear": 0, "signals": {}, "details": {}}
            df = price["df"]
        return _rva(symbol, df)
    except Exception as e:
        _logger.warning("run_vibe_agents error %s: %s", symbol, e)
        return {"bull": 0, "bear": 0, "signals": {}, "details": {}}
