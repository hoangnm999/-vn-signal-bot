...
# ══════════════════════════════════════════════════════════════════════════════
# DATA LAYER — vnstock 3.x API
# ══════════════════════════════════════════════════════════════════════════════

def _get_stock_obj(symbol: str):
...
def get_fundamental_data(symbol: str) -> dict:
    try:
        stock = _get_stock_obj(symbol)
        # vnstock 3.x: stock.finance.ratio()
        ...
        return {
            ...
            "pe":             round(_f("P/E"), 2),
            "pb":             round(_f("P/B"), 2),
            "roe":            round(_f("ROE") * 100, 2),
            "eps":            round(_f("EPS"), 0),
            "revenue_growth": round(_f("Revenue Growth") * 100, 2),
            "profit_growth":  round(_f("Profit Growth") * 100, 2),
            "debt_equity":    round(_f("Debt/Equity"), 2),
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def get_foreign_flow_data(symbol: str) -> dict:
    try:
        ...
        foreign_cols = [c for c in df.columns if "foreign" in c or "nn" in c]
        buy_col  = next((c for c in foreign_cols if "buy"  in c), None)
        sell_col = next((c for c in foreign_cols if "sell" in c), None)

        if not buy_col or not sell_col:
            return {"success": False, "error": "Không có cột foreign flow trong data nguồn VCI"}

        df["net"] = df[buy_col] - df[sell_col]
        return {
            "success":   True,
            "net_today": round(float(df["net"].iloc[-1]) / 1e9, 2),
            "net_5d":    round(float(df["net"].tail(5).sum()) / 1e9, 2),
            "net_20d":   round(float(df["net"].tail(20).sum()) / 1e9, 2),
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def get_market_data() -> dict:
    try:
        ...
        # VN-Index dùng source TCBS hoặc VCI
        ...
        return {
            "success":    True,
            "vnindex":    round(float(curr), 2),
            "change_5d":  round((float(curr) - float(close.iloc[-5])) / float(close.iloc[-5]) * 100, 2) if len(close) >= 5 else 0,
            "change_20d": round((float(curr) - float(close.iloc[-20])) / float(close.iloc[-20]) * 100, 2) if len(close) >= 20 else 0,
...
def get_news_data(symbol: str) -> dict:
    """
    Thu thập tin tức & thảo luận từ 12 nguồn song song:
      1. Google News RSS  2. CafeF       3. f319        4. Fireant
      5. VCSC Research    6. HSC Research 7. ACBS        8. VnEconomy
      9. VNDIRECT         10. SSI         11. Stockbiz   12. Vietstock
    """
    import re
    ...
    def scrape_f319():
        try:
            ...
            if r.status_code == 200:
                ...
                results = [f"[f319] {clean(t)}" for t in titles[:5] if clean(t)]
                return results
        except Exception:
            pass
        return []
    ...
    def scrape_vndirect():
        try:
            ...
            if r.status_code == 200:
                titles = re.findall(r'<h\d[^>]*>(.*?)</h\d>', r.text, re.DOTALL)
                results = [f"[VNDIRECT] {clean(t)[:120]}" for t in titles[:4] if len(clean(t)) > 10]
                return results
        except Exception:
            pass
        return []
    ...
    # ── Chạy song song 12 nguồn ───────────────────────────────────────────
    sources      = [scrape_google_news, scrape_cafef, scrape_f319, scrape_fireant,
                    scrape_vcsc, scrape_hsc, scrape_acbs, scrape_vneconomy,
                    scrape_vndirect, scrape_ssi, scrape_stockbiz, scrape_vietstock]
    source_names = ["GoogleNews", "CafeF", "f319", "Fireant",
...
def compute_indicators(df: pd.DataFrame) -> dict:
    ...
    p1w  = float(close.iloc[-5])  if len(close) >= 5  else float(close.iloc[0])
    p1m  = float(close.iloc[-20]) if len(close) >= 20 else float(close.iloc[0])
...
def run_news_agent(symbol, news):
    ...
    sys_p = ("Bạn là chuyên gia phân tích sentiment đa nguồn cho TTCK VN. "
             ...
             "báo cáo CTCK chuyên nghiệp (VCSC/HSC/ACBS/VNDIRECT/SSI). "
...
def run_verdict_agent(symbol, verdicts):
    sys_p = ("Bạn là Chief Investment Analyst, tổng hợp ý kiến từ 6 chuyên gia độc lập. "
