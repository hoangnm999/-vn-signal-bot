...
def run_volume_agent(symbol, ind):
    sys_p = ("Bạn là chuyên gia phân tích khối lượng giao dịch chứng khoán VN. "
             ...
             "QUY TẮC BẮT BUỘC: Nếu Volume/TB20 < 0.5x thì KẾT LUẬN PHẢI là NGHI NGỜ (volume quá thấp không thể xác nhận xu hướng). "
...
def run_fundamental_agent(symbol, fund):
    ...
    sys_p = ("Ban la chuyen gia tai chinh co kien thuc ve cac cong ty niem yet tren HOSE/HNX. "
             ...
             "Viet NGAN GON toi da 4 cau de tranh bi cat. "
             "Dong cuoi PHAI la — Ket luan: TOT hoac TRUNG TINH hoac YEU")
    user = (f"Co phieu: {symbol}\n"
            ...
            f"nganh nghe, vi the canh tranh, lich su KQKD, danh gia fundamental toi da 4 cau. "
            f"Neu khong biet ro ve cong ty nay thi noi 'Khong du thong tin de danh gia'. "
            f"Ket luan: TOT/TRUNG TINH/YEU")
    txt = call_deepseek(sys_p, user, max_tokens=600)
    v = "TOT" if "TOT" in txt.upper() else "YEU" if "YEU" in txt.upper() else "TRUNG TINH"
...
CHI TIET AGENTS:
{_fmt_agent("Xu huong",    trend_v,  trend_txt)}

{_fmt_agent("Volume",      volume_v, volume_txt)}

{_fmt_agent("Rui ro",      risk_v,   risk_txt)}

{_fmt_agent("Fundamental", fund_v,   fund_txt)}

{_fmt_agent("Smart Money", smart_v,  smart_txt)}

{_fmt_agent("News",        news_v,   news_txt)}

{_fmt_agent("Market",      market_v, market_txt)}

{_fmt_agent("Macro",       macro_v,  macro_txt)}

{'='*30}
{emoji} KET LUAN TONG HOP: {final_v}
   Do tin cay: {conf}/10 | Dong thuan: {bull}/{n_agents} bullish, {bear}/{n_agents} bearish

LY DO CHINH: {summary}

{_format_action_plan(final_v, ap)}

LUU Y RUI RO: {negative}
{'='*30}""".strip()
