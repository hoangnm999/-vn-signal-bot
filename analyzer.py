...
def _fundamental_from_entrade(symbol: str) -> dict:
    """
    Lấy fundamental từ Entrade chart-api.
    Entrade trả về income statement + balance sheet riêng.
    Từ đó tính PE, ROE, EPS thủ công.
    """
    import time
    to_ts   = int(time.time())
    from_ts = to_ts - 3 * 365 * 86400  # 3 năm

    # Income Statement (năm)
    r_inc = requests.get(
        f"https://services.entrade.com.vn/chart-api/v2/incomestatement"
        f"?symbol={symbol}&resolution=Y",
        headers=_BROWSER_HEADERS, timeout=10
    )

    # Balance Sheet (năm)
    r_bal = requests.get(
        f"https://services.entrade.com.vn/chart-api/v2/balancesheet"
        f"?symbol={symbol}&resolution=Y",
        headers=_BROWSER_HEADERS, timeout=10
    )

    if r_inc.status_code != 200 and r_bal.status_code != 200:
        raise ValueError(f"Entrade income {r_inc.status_code}, balance {r_bal.status_code}")

    def _safe(data, *keys):
        """Lấy giá trị cuối của series trong JSON"""
        for k in keys:
    ...
    # Keys Entrade income statement: revenue, netProfit, eps, ...
    revenue     = _safe(inc, "revenue", "netRevenue")
    net_profit  = _safe(inc, "netProfit", "profit")
    eps         = _safe(inc, "eps", "EPS")
    equity      = _safe(bal, "equity", "ownerEquity")
    total_debt  = _safe(bal, "totalDebt", "liabilities")

    # Revenue growth: (năm n - năm n-1) / năm n-1
    rev_arr = inc.get("revenue") or inc.get("netRevenue") or []
    rev_growth = 0.0
    if isinstance(rev_arr, list) and len(rev_arr) >= 2:
        try:
            r1, r0 = float(rev_arr[-1]), float(rev_arr[-2])
            rev_growth = (r1 - r0) / abs(r0) if r0 else 0
        except Exception:
            pass

    profit_arr = inc.get("netProfit") or inc.get("profit") or []
    profit_growth = 0.0
    if isinstance(profit_arr, list) and len(profit_arr) >= 2:
        try:
            p1, p0 = float(profit_arr[-1]), float(profit_arr[-2])
            profit_growth = (p1 - p0) / abs(p0) if p0 else 0
        except Exception:
            pass
    ...
    return {
        "success":        True,
        "pe":             0.0,   # cần giá thị trường, skip
        "pb":             0.0,
        "roe":            round(roe, 2),
        "eps":            round(eps, 0),
...
def _fundamental_from_vnstock(symbol: str):
    """Lấy ratio từ vnstock KBS source"""
    from vnstock import Vnstock
...
def get_fundamental_data(symbol: str) -> dict:
    """Multi-source: vnstock KBS → Entrade income/balance → fail gracefully"""

    # Source 1: vnstock KBS ratio (nhanh, đầy đủ nhất)
    try:
    ...
    # Source 2: Entrade income + balance sheet
    try:
        return _fundamental_from_entrade(symbol)
    except Exception as e:
        return {"success": False, "error": f"Không lấy được fundamental ({str(e)[:60]})"}


def _foreign_from_entrade(symbol: str, days: int) -> dict:
    """
    Lấy foreign flow từ Entrade.
    Entrade foreign endpoint: /chart-api/v2/foreign?symbol=VCB&resolution=D
    Response keys: fBuyVal, fSellVal (hoặc buyVal, sellVal)
    """
    import time
    to_ts   = int(time.time())
    from_ts = to_ts - days * 86400
    ...
    endpoints = [
        f"https://services.entrade.com.vn/chart-api/v2/foreign?symbol={symbol}&resolution=D&from={from_ts}&to={to_ts}",
    ...
    for url in endpoints:
        try:
            ...
            # Tìm key buy/sell theo nhiều naming convention
            buy_key  = next((k for k in data
                             if any(p in k.lower() for p in ["fbuyval","buyval","foreignbuy","buy_val"])
                             and isinstance(data[k], list)), None)
            sell_key = next((k for k in data
                             if any(p in k.lower() for p in ["fsellval","sellval","foreignsell","sell_val"])
                             and isinstance(data[k], list)), None)

            if not buy_key or not sell_key:
                continue

            buy_arr  = [float(x) for x in data[buy_key]]
            sell_arr = [float(x) for x in data[sell_key]]
            ...
            return {
                "success":   True,
                "net_today": round(net_arr[-1]            / 1e9, 2),
                "net_5d":    round(sum(net_arr[-5:])      / 1e9, 2) if len(net_arr) >= 5  else round(sum(net_arr) / 1e9, 2),
                "net_20d":   round(sum(net_arr[-20:])     / 1e9, 2) if len(net_arr) >= 20 else round(sum(net_arr) / 1e9, 2),
                "source":    url.split("?")[0].split("/")[-1],
            }
        except Exception:
            continue

    raise ValueError("Không lấy được foreign flow từ Entrade (tất cả endpoints fail)")
...
def get_news_data(symbol: str) -> dict:
    ...
    def fetch_google_news():
        """Google News RSS — bao gồm tất cả báo VN, f319, CTCK"""
        results = []
        queries = [
            f"{symbol} cổ phiếu",
            f"{symbol} chứng khoán kết quả kinh doanh",
            f"{symbol} VCSC HSC báo cáo phân tích",  # bù cho báo cáo CTCK
            f"f319 {symbol}",                          # bù cho f319
        ]
        for q in queries:
            url = f"https://news.google.com/rss/search?q={urllib.parse.quote(q)}&hl=vi&gl=VN&ceid=VN:vi"
            items = parse_rss(url, "GoogleNews", 4)
