...
def get_news_data(symbol: str) -> dict:
    ...
    def fetch_fireant_api():
        """
        Fireant — lấy từ 2 endpoint song song:
          1. /posts?symbol=   → bình luận cộng đồng trader
          2. /symbols/{sym}/news → tin tức cơ bản từ báo chí
        Cần FIREANT_TOKEN (Bearer). Fallback web scrape khi 401/không có token.
        Cache nội bộ 10 phút để tránh hammer API.
        """
        import time as _time
        cache_key = f"_fa_cache_{symbol}"
        cached = getattr(fetch_fireant_api, cache_key, None)
        ...
        if FIREANT_TOKEN:
            fa_headers["Authorization"] = f"Bearer {FIREANT_TOKEN}"

        results = []

        # ── Endpoint 1: Social posts ──────────────────────────────────────
        try:
            r = requests.get(
                f"https://restv2.fireant.vn/posts?symbol={symbol}&limit=15&offset=0",
                headers=fa_headers, timeout=8,
            )
            if r.status_code == 200:
                posts = r.json()
                ...
                for p in posts[:10]:
                    ...
                    if content and len(content) > 20:
                        likes = p.get("likeCount") or p.get("totalLikes") or 0
                        hint  = _fa_sentiment(content)
                        tag   = f"[Fireant/{hint}]" if hint != "neutral" else "[Fireant]"
                        like_str = f" 👍{likes}" if likes > 5 else ""
                        results.append(f"{tag} {content[:180]}{like_str}")
            elif r.status_code == 401:
                results.extend(_fetch_fireant_web(symbol))
        except Exception:
            pass
        ...
        if FIREANT_TOKEN:
            try:
                r2 = requests.get(
                    f"https://restv2.fireant.vn/symbols/{symbol}/news?limit=10&offset=0",
                    headers=fa_headers, timeout=8,
                )
                if r2.status_code == 200:
                    news_items = r2.json()
                    if not isinstance(news_items, list):
                        news_items = news_items.get("items", news_items.get("data", []))
                    for item in news_items[:8]:
                        title = (item.get("title") or item.get("name") or "").strip()
                        title = re.sub(r"<[^>]+>", "", title).strip()
                        if title and len(title) > 15:
                            hint = _fa_sentiment(title)
                            tag  = f"[Fireant-News/{hint}]" if hint != "neutral" else "[Fireant-News]"
                            results.append(f"{tag} {title[:200]}")
            except Exception:
                pass

        setattr(fetch_fireant_api, cache_key, (_time.time(), results))
        return results

    def _fa_sentiment(text: str) -> str:
        """Quick keyword sentiment cho Fireant content."""
        t = text.lower()
        bull = sum(1 for kw in [
            "tăng", "mua", "tích lũy", "breakout", "bứt phá",
            "lợi nhuận tăng", "kết quả tốt", "hỗ trợ tốt", "target",
        ] if kw in t)
        bear = sum(1 for kw in [
            "giảm", "bán", "cắt lỗ", "breakdown", "rủi ro",
            "lỗ", "xả", "thủng", "margin call",
        ] if kw in t)
    ...
    def fetch_f319_html():
        ...
        3 chiến lược theo thứ tự:
          1. Google site:f319.com search (bypass CF, không cần token)
          2. F319 internal search endpoint
          3. F319 forum chuyên mục mã cổ phiếu

        Cache 5 phút để tránh bị block.
        Thêm sentiment hint vào mỗi kết quả.
        """
        import time as _time
        import urllib.parse as _urlparse

        cache_key = f"_f319_cache_{symbol}"
        cached = getattr(fetch_f319_html, cache_key, None)
        ...
        def _add(raw: str, max_len: int = 180):
            """Thêm vào results nếu chưa có và đủ dài."""
            c = _clean(raw)
            if len(c) > 15 and c not in _seen:
                _seen.add(c)
                hint = _f319_sentiment(c)
                tag  = f"[f319/{hint}]" if hint != "neutral" else "[f319]"
                results.append(f"{tag} {c[:max_len]}")

        def _f319_sentiment(text: str) -> str:
            t = text.lower()
            bull = sum(1 for kw in [
                "tăng", "mua", "tích lũy", "break", "bứt phá",
                "hỗ trợ", "uptrend", "target", "sóng tăng",
            ] if kw in t)
            bear = sum(1 for kw in [
                "giảm", "bán", "cắt lỗ", "phá đáy", "downtrend",
                "rủi ro", "xả", "thủng", "bearish",
            ] if kw in t)
            if bull > bear: return "bullish"
            if bear > bull: return "bearish"
            return "neutral"
        ...
        # ── Chiến lược 1: Google site:f319.com ───────────────────────────
        try:
            q   = _urlparse.quote_plus(f'site:f319.com "{symbol}"')
            url = f"https://www.google.com/search?q={q}&num=15&hl=vi"
            r   = requests.get(url, headers=_HEADERS_BASE, timeout=10)
            if r.status_code == 200:
                try:
                    ...
                    for g in soup.select("div.g"):
                        h3 = g.select_one("h3")
                        if h3 and "f319" in (g.select_one("a") or {}).get("href", ""):
                            _add(h3.get_text())
                        if len(results) >= 6:
                            break
                except ImportError:
                    # bs4 không có → regex fallback
        ...
        # ── Chiến lược 2: F319 internal search ───────────────────────────
        if len(results) < 4:
            try:
                # Thử cloudscraper trước (bypass CF), fallback requests
                try:
                    ...
                    r2 = sess.get(
                        f"https://f319.com/search/results?query={_urlparse.quote(symbol)}&t=post",
                        headers={"Accept-Language": "vi-VN,vi;q=0.9"},
                        timeout=12,
                    )
                except ImportError:
                    r2 = requests.get(
                        f"https://f319.com/search?q={_urlparse.quote(symbol)}&t=post",
                        headers={**_HEADERS_BASE, "Referer": "https://f319.com/"},
                        timeout=10,
                    )
                ...
                if r2.status_code == 200:
                    ...
                    try:
                        ...
                        for sel in [
                            ...
                            "li.searchResult h3 a",
                            ".block-row .contentRow-title a",
                            "h3.title a",
                        ]:
                            for el in soup2.select(sel)[:8]:
                                _add(el.get_text())
                            if len(results) >= 6:
                                break
                    except ImportError:
        ...
        # ── Chiến lược 3: F319 forum chuyên mục ──────────────────────────
        if len(results) < 3:
            try:
                r3 = requests.get(
                ...
                if r3.status_code == 200:
                    try:
                        from bs4 import BeautifulSoup as _BS
                        soup3 = _BS(r3.text, "lxml")
                        for el in soup3.select(
                            "h3.structItem-title a, .structItem-title a, h3.title a"
                        )[:8]:
                            _add(el.get_text())
                    except ImportError:
                        for pat in [
                            ...
                            for m in re.findall(pat, r3.text, re.IGNORECASE | re.DOTALL):
                                _add(m)
            except Exception:
                pass

        setattr(fetch_f319_html, cache_key, (_time.time(), results))
        return results[:8]
    ...
    source_names = [
        ...
        "NDH", "DanTri", "ThanhNien", "TNCK", "Fireant+News", "f319",
    ]
