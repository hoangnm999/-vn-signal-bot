...
def _build_sr_debug_report(symbol: str) -> str:
    ...
    try:
        from stock_regime import (
            ...
            _predict_current, SR_LABELS, GMM_DIMS,
        )
        from vn_loader import load_vn_ohlcv
    ...
    lines.append(f"  (dung {len(GMM_DIMS)}/15 dims, loai vol_trend/atr_ratio/high_low_pos/candle_body)")

    # Sort theo SR label để dễ đọc
    centroid_rows = []
    for c_idx, sr_idx in label_mapping.items():
        c = centroids[c_idx]   # shape (len(GMM_DIMS),) — đã subset
        from stock_regime import _GMM_IDX_MOMENTUM_5D, _GMM_IDX_MOMENTUM_20D
        from stock_regime import _GMM_IDX_TREND_SLOPE, _GMM_IDX_PRICE_SMA20, _GMM_IDX_VOLUME_SPIKE
        bull_score = float(c[_GMM_IDX_MOMENTUM_5D] + c[_GMM_IDX_MOMENTUM_20D]
                           + c[_GMM_IDX_TREND_SLOPE] + c[_GMM_IDX_PRICE_SMA20])
        volume     = float(c[_GMM_IDX_VOLUME_SPIKE])
        trend      = float(c[_GMM_IDX_TREND_SLOPE])
    ...
    sep_ok = (bull_vals.get(2, 0) > bull_vals.get(4, 0))   # điều kiện tối thiểu
    sep_ideal = (bull_vals.get(2,0) > bull_vals.get(3,0) >
                 bull_vals.get(1,0) > bull_vals.get(4,0))
    if sep_ideal:
        sep_note = "✅ SR2>SR3>SR1>SR4 (ly tuong)"
    elif sep_ok:
        sep_note = "🟡 SR2>SR4 (chap nhan duoc, SR1/SR3 co the dao)"
    else:
        sep_note = "⚠️ SR2<=SR4 — GMM validity fail, se hien 'Khong xac dinh'"
    lines.append("")
    lines.append(f"  BullScore ordering: {sep_note}")
    ...
    ll_current = None
    try:
        x_subset = current_vec[GMM_DIMS]
        x_scaled = scaler.transform(x_subset.reshape(1, -1))
        ll_current = float(gmm.score_samples(x_scaled)[0])
        ll_note = "✅ Binh thuong" if ll_current > -20 else f"⚠️ OUTLIER (ll={ll_current:.1f})"
    ...
    lines.append("")
    ll_ok  = ll_current is None or ll_current > -20
    if conf >= 0.9999:
        if sep_ok and ll_ok:
            # Cluster rõ ràng, vector nằm sâu trong SR — kết quả tin cậy
            lines.append(
                f"  ✅ Confidence 100% — HAH nam sau trong vung SR{sr}, "
                f"cluster tach biet ro rang"
            )
        else:
            # Degenerate hoặc outlier
            lines.append("  ⚠️  prob=100% nhung co dau hieu degenerate:")
            if not ll_ok:
                lines.append(f"     Log-likelihood {ll_current:.1f} < -20 (outlier)")
            if not sep_ok:
                lines.append("     BullScore ordering sai (SR2 <= SR4)")
    elif conf >= 0.75:
