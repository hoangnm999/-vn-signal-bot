...
def _fit_gmm(vectors: np.ndarray) -> Optional[tuple]:
    ...
    Covariance type = 'diag' thay vì 'full':
      - 'full': mỗi cluster có covariance matrix riêng (15×15) → dễ bị 1
        cluster quá hẹp, prob 100% cho điểm gần tâm
      - 'diag': chỉ fit variance từng dimension, không fit correlation →
        phân phối đồng đều hơn, robust hơn với data ít (~400 vectors)
      Trade-off: mất thông tin correlation giữa dims, nhưng đổi lại
      stability quan trọng hơn với TTCK VN thanh khoản thấp.

    Label validity check:
      Sau khi assign label, kiểm tra BullScore ordering có hợp lý không.
      Nếu sai (data không đủ 4 giai đoạn rõ ràng) → trả về None với
      thông báo rõ ràng thay vì gán label sai.

    Returns:
        (gmm_model, scaler, label_mapping, centroids) hoặc None nếu fail
    """
    try:
...
def _compute_stock_regime(symbol: str, df: pd.DataFrame) -> dict:
    ...
    if fit_result is None:
        # Có thể do data không đủ 4 giai đoạn rõ ràng
        err_result["error"] = (
            f"Khong the phan loai 4 giai doan ro rang tu {len(vectors)} vectors. "
            f"Co the {symbol} chua trai qua du Markup/Markdown trong lich su gan day."
        )
        err_result["regime"]           = 0   # 0 = không xác định
        err_result["label"]            = "Khong xac dinh"
        err_result["emoji"]            = "⚪"
        err_result["desc"]             = "Khong du lich su de phan loai Stock Regime"
        err_result["action"]           = "Dung /check va /wave de phan tich them"
        err_result["confidence_label"] = "Khong xac dinh"
        return err_result
...
def format_stock_regime_inline(sr_data: dict) -> str:
    ...
    """
    if not sr_data.get("ok") or sr_data.get("regime", 0) == 0:
        err = sr_data.get("error", "N/A")[:60]
        return f"⚪ Stock Regime: Khong xac dinh ({err})"
...
def format_stock_regime_for_wave(sr_data: dict) -> str:
    ...
    if not sr_data.get("ok") or sr_data.get("regime", 0) == 0:
        return "⚪ StockRegime: Khong xac dinh (thieu lich su 4 giai doan)"
    ...
    if r == 2:
        ...
        note = "→ Tich luy: xac nhan kha nang song tang tu day"
    elif r == 3:
        note = "→ Phan phoi: song tang co the la dinh cuoi"
