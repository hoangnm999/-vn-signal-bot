...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "/backtest_rule <MA> \"entry_rule\" \"exit_rule\" — Backtest rule tu Vibe\n"
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "/backtest_rule <MA> \"<entry>\" \"<exit>\" [ngay]\n"
        "Backtest rule tu hoi dong chuyen gia Vibe:\n"
        "  Vi du: /backtest_rule VCB \"rsi < 30\" \"rsi > 70\"\n"
        "  Vi du: /backtest_rule STB \"close < 62100 and rsi < 30\" \"close > 68000\"\n\n"
...
# ── /debug_loader <MA> — kiểm tra VN data sources ─────────────────────────────
async def debug_loader_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Kiểm tra tất cả data sources của vn_loader — debug backtest data."""
    if not is_allowed(update): await _deny(update); return
    args = context.args or []
    symbol = args[0].upper() if args else "VCB"
    valid, symbol = _validate_symbol(symbol)
    if not valid:
        await update.message.reply_text("Ma khong hop le."); return

    msg = await update.message.reply_text(f"Dang kiem tra data sources cho {symbol}... (15-30s)")
    try:
        from vn_loader import debug_sources
        result = await asyncio.to_thread(debug_sources, symbol, 100)
        await _edit_or_split(msg, update.message, result)
    except Exception as e:
        await msg.edit_text(f"Loi debug_loader: {str(e)[:200]}")
