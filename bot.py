...
try:
    ...
    _VN_LOADER = False
    logger.warning("vn_loader.py chua co — /debug_loader bi tat")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "/deepscan      — Phan tich sau 13 engines toan bo watchlist\n"
        ...
        "/alert <MA> <gia> [above|below] — Dat canh bao gia\n"
        ...
        "/local_swarm <MA> — Hoi dong chuyen gia AI noi bo (5 experts, 2 vong)\n"
...
async def debug_loader_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _VN_LOADER:
        await update.message.reply_text("vn_loader.py chua co."); return

    msg = await update.message.reply_text(f"Dang kiem tra sources cho {symbol}... (~30s)")
    try:
        report = await asyncio.to_thread(_vn_debug_sources, symbol, 100)
        lines = [f"=== DEBUG LOADER {symbol} ==="]
        if isinstance(report, dict):
            for src, info in report.items():
                status = "OK" if info.get("ok") else f"FAIL: {info.get('error','?')[:60]}"
                bars   = f" | {info.get('bars',0)} bars" if info.get("ok") else ""
                time_  = f" ({info.get('time',0):.1f}s)" if "time" in info else ""
                lines.append(f"  {src}: {status}{bars}{time_}")
        else:
    ...
    async def _run_swarm_bg():
        try:
            ...
            try:
                ...
                # get_price_data trả về dict {"success": bool, "df": DataFrame}
                price_result = await asyncio.to_thread(get_price_data, symbol, 300)
                df = None
                if isinstance(price_result, dict) and price_result.get("success"):
                    df = price_result.get("df")
                elif hasattr(price_result, "__len__"):
                    # fallback nếu trả thẳng df
                    df = price_result

                if df is not None and len(df) >= 60:
                    check_data = await asyncio.to_thread(run_vibe_agents, symbol, df)
                    ind        = await asyncio.to_thread(get_indicators, symbol, df)
                    # Đảm bảo check_data là dict (run_vibe_agents đôi khi trả str khi lỗi)
                    if not isinstance(check_data, dict):
                        check_data = {"bull": 0, "bear": 0, "signals": {}, "details": {}}
