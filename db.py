...
def get_conn():
    ...
    if not db_url:
        raise RuntimeError("DATABASE_URL chưa được set trong Railway Variables")
    ...
    try:
        ...
        pass  # psycopg2 không có → thử pg8000
    except Exception as e:
        raise RuntimeError(f"psycopg2 connect failed: {e}")

    # Fallback: pg8000 (pure Python, luôn cài được trên mọi platform)
    try:
        import pg8000.native
        import urllib.parse
        p = urllib.parse.urlparse(db_url)
        # pg8000 dùng SSL mặc định cho Railway
        ...
        # Wrap pg8000 để có interface giống psycopg2
        return _Pg8000Wrapper(conn)
    except ImportError:
        raise RuntimeError(
            "Khong the ket noi PostgreSQL: ca psycopg2 va pg8000 deu chua duoc cai.\n"
            "Kiem tra requirements.txt co 'psycopg2-binary' hoac 'pg8000'."
        )
    except Exception as e:
...
class _Pg8000Wrapper:
    """Wrap pg8000 Native connection để có interface cursor() giống psycopg2."""
    def __init__(self, conn):
        self._conn = conn

    def cursor(self):
        return _Pg8000Cursor(self._conn)
...
class _Pg8000Cursor:
    """Minimal cursor wrapper cho pg8000."""
    def __init__(self, conn):
        self._conn = conn
        self._rows = []
        self._columns = []
    ...
    def execute(self, sql, params=None):
        ...
        self._columns = [c["name"] for c in self._conn.columns] if self._conn.columns else []
