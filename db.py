...
-- Đảm bảo INDEX cơ bản tồn tại sau rename
CREATE INDEX IF NOT EXISTS idx_signals_symbol   ON signals(symbol);
CREATE INDEX IF NOT EXISTS idx_signals_created  ON signals(created_at);

-- Migration: thêm cột expires_at nếu chưa có (cho DB cũ không có cột này)
-- QUAN TRỌNG: CREATE INDEX expires phải đặt SAU block này
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        ...
        WHERE expires_at = (NOW() + INTERVAL '5 days');
        RAISE NOTICE 'Migration: expires_at column added and backfilled.';
    END IF;
END $$;

-- Index expires_at đặt SAU migration để tránh lỗi "column does not exist"
...
def init_db():
    ...
    # Khởi tạo bảng portfolio (tách riêng để lỗi không block bot chính)
    try:
