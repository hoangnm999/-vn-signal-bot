...
class FundamentalFilterEngine:
    ...
    def _one(self, df: pd.DataFrame, code: str) -> tuple:
        # Thử từng source theo thứ tự, lấy kết quả đầu tiên có đủ data
        fund = {}
        for fetch_fn, label in [
            (self._fetch_fundamentals_vnstock,  "vnstock"),
            (self._fetch_fundamentals_entrade,  "Entrade"),
            (self._fetch_fundamentals_cafef,    "CafeF"),
            (self._fetch_fundamentals_yfinance, "yfinance"),
        ]:
            try:
                fund = fetch_fn(code)
            except Exception:
