...
def _scan_symbol(symbol: str, cluster: str) -> dict | None:
    ...
    # Signal history 20 ngày — anti-chasing context
    # Gọi sau khi signal đã confirmed (tránh query DB cho signal bị block)
    sig_hist = _get_signal_history(symbol, cluster, days=20)
    ...
    return {
        ...
        "signal_history": sig_hist,          # S38: anti-chasing context (20d window)
    }
...
def _format_signal(sig: dict, vni_info: dict,
    ...
    # S38: Signal history 20 ngày — anti-chasing context
    # Chỉ hiển thị khi signal_count >= 2 (lần đầu = không noise)
    ...
    if hist and hist.get("count", 0) >= 2:
        ...
        fd      = hist["first_date"]
        ep      = sig["entry_price"]
        chg     = (ep / fp - 1) * 100 if fp > 0 else 0
        chg_str = f"{chg:+.1f}%" if fp > 0 else "?"
        warn    = " ⚠️ ĐUỔI GIÁ" if chg > 5 else (" 🔴 GIÁ ĐÃ TĂNG NHIỀU" if chg > 10 else "")
        lines.append(
            f"📍 Signal #{n}/20d | Lần đầu: {fd} ~{fp:,.0f} ({chg_str} vs nay){warn}"
        )
...
def _get_signal_history(symbol: str, cluster: str, days: int = 20) -> dict:
    """
    Trả về lịch sử signal của (symbol, cluster) trong `days` ngày gần nhất.

    Returns:
        {
            "count":       int,    # số lần signal (kể cả hôm nay)
            "first_price": float,  # entry price lần đầu tiên trong window
            "first_date":  str,    # ngày signal đầu tiên (dd/mm/YYYY)
            "price_chg":   float,  # % thay đổi giá so với lần đầu (entry hôm nay / first_price)
        }
        Trả về {} nếu không query được hoặc không có lịch sử.
    """
    try:
        ...
        rows = journal_get_active()   # trả về list dict với entry_date, symbol, cluster, entry_price
        # Lọc theo symbol + cluster + trong window
        hist = [
            r for r in rows
            if r.get("symbol") == symbol
            and r.get("cluster") == cluster
            and r.get("entry_date") is not None
            and (r["entry_date"] if isinstance(r["entry_date"], _date)
                 else _date.fromisoformat(str(r["entry_date"]))) >= cutoff
        ]
        if not hist:
            return {}

        # Sắp xếp theo ngày tăng dần
        hist.sort(key=lambda r: r["entry_date"])
        first      = hist[0]
        first_price = float(first.get("entry_price") or 0)
        first_date  = (first["entry_date"] if isinstance(first["entry_date"], _date)
                       else _date.fromisoformat(str(first["entry_date"])))
        return {
            "count":       len(hist),
        ...
        logger.debug(f"[SignalHistory] {symbol}/{cluster}: {e}")
        return {}
