...
async def size_cmd(update, context):
    ...
    # Chỉ remove dấu phẩy (separator), giữ dấu chấm thập phân
    support_raw = args[1].replace(",", "")
    ...
    if len(args) >= 4:
        try:
            risk_filter = float(args[3].replace(",", ""))
        except ValueError:
            pass
    ...
    # Format account size cho display
    if account >= 1_000_000_000:
        account_str = f"{account/1e9:.1f}tỷ"
    else:
        account_str = f"{account/1e6:.0f}tr"
    ...
    # Note nếu symbol trong MR/MR2 (dynamic SL active)
    all_syms = {
